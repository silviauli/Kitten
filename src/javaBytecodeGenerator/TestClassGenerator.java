...
public class TestClassGenerator extends JavaClassGenerator {
	...
	public void createMain() {
		...
		for (TestSignature test: clazz.getTests()) {
			...
			// create the test
			ilTest = tryTest(test, clazz.getFixtures());
		...
		// # tests passed ritorna i test passati
		iList.append(getFactory().createGetStatic("java/lang/System", "out", 
				Type.getType(java.io.PrintStream.class)));
		iList.append(InstructionFactory.ILOAD_1); //aggiunge sullo stack il numero di test falliti
		iList.append(new LDC(getConstantPool().addInteger(clazz.getTests().size()))); //numero di test totali
		iList.append(InstructionFactory.IADD); //aggiunge sullo stack il numero di test passati
		...
		iList.append(tryPrint(" test(s) passed, "));
		
		// # tests failed
		iList.append(getFactory().createGetStatic("java/lang/System", "out", 
				Type.getType(java.io.PrintStream.class)));
		iList.append(InstructionFactory.ILOAD_1); //aggiunge sullo stack il numero di test falliti
		iList.append(InstructionFactory.INEG); //nega il valore per ritornare i test falliti
		iList.append(getFactory().createInvoke(
				"java/io/PrintStream", 
				"print", 
				Type.VOID, 
	...
	private void tryTime(InstructionList il) {
	...
	private InstructionList tryPrint(String msg) {
	...
	private InstructionList tryTest(TestSignature test, Set<FixtureSignature> fixtures) {
		InstructionList il = new InstructionList();
		il.append(tryPrint("\t- Test: " + test.getName() + " "));
